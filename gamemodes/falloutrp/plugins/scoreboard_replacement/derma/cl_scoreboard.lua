	...
	function PANEL:addPlayer(client, parent)
		...
		slot.model.DoClick = function()
			local menu = DermaMenu()
				...
				local optionsC = table.Count(options)
				if (optionsC > 0) then
					local count = 0
					for k, v in SortedPairs(options) do
						...
						if (v == "spacer") then
							if (count < optionsC) then
								menu:AddSpacer()
							end
						elseif (istable(v[2])) then
							local submenu, submenuParent = menu:AddSubMenu(L(k))
							submenuParent:SetIcon(v[1])
							for k2, v2 in SortedPairs(v[2]) do
								local subOption = submenu:AddOption(L(k2), v2[2])
								if (v2[1] and v2[1] != "") then
									subOption:SetIcon(v2[1])
								end
							end
						else
							menu:AddOption(L(k), v[2]):SetIcon(v[1])
						end
					end
				end
			menu:Open()
