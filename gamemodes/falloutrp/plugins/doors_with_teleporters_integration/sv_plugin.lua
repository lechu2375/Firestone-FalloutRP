...
function PLUGIN:LoadData()
	...
	for k, v in pairs(data) do
		...
		if isnumber(k) then
			entity = ents.GetMapCreatedEntity(k)
		else
			local id =  tonumber(string.Right(k, #k - 2))
			
			for _, ent in pairs(ents.FindByClass("forp_teleport")) do
				if ent.doorindex == id then
					entity = ent
					break
				end
			end
...
function PLUGIN:SaveDoorData()
	...
	local data = {}
		...
		for k, v in ipairs(ents.GetAll()) do
			if (v:isFakeDoor()) then
				doors["f_"..v.doorindex] = v
