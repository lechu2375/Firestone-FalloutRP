    ...
    function BodyGroupsConvert(modelEnt)
        local ctable = {}
        ctable.bodygroups = ""
        ctable.model = modelEnt:GetModel()
        for i=0,modelEnt:GetNumBodyGroups() do
            ctable.bodygroups=(ctable.bodygroups..(modelEnt:GetBodygroup(i)))
        end
        return ctable
    end
    ...
    function basePANEL:Init()
        ...
        ModelPanel:SetAnimated( true ) --Lepiej dodać niż nowe zrobić bo będzie zablurowane
        ...
        --FOCUS ON BONE
            ...
            --GENEROWANIE PRZYCISKÓW DLA DOSTĘPNYCH BODYGROUPOW
                ...
                --Zmiana BODYGROUPA NA WYŻSZY END
                --obrót fajny
                local angleSlider = ModelPanel:Add("DNumSlider")
                angleSlider:SetPos(10,ModelPanel:GetTall()-100)
                angleSlider:SetSize(ModelPanel:GetWide(), 50)
                angleSlider:SetText("Obrót")
                angleSlider:SetMin(0)
                angleSlider:SetMax(360)
                angleSlider:SetDecimals(0)
                angleSlider:SetValue((ModelPanel.Entity:GetAngles().yaw))
                angleSlider.OnValueChanged = function(self, value)
                    ModelPanel.Entity:SetAngles((Angle(0,value,0)))
                end   
                --obrót end             
                --Przełącz obrót BUTTON 
                    local toggleTurn = self:Add("FS_Button")
                    local posx, posy = buttonRandomAnim:GetPos()
                    toggleTurn:SetPos(posx,(posy+buttonRandomAnim:GetTall())+2)
                    toggleTurn:SetSize( buttonRandomAnim:GetSize() )
                    toggleTurn:SetText("Obrót +")
                    function toggleTurn.DoClick() 
                        ModelPanel.Entity:SetAngles((ModelPanel.Entity:GetAngles()+Angle(0,20,0)))
                        angleSlider:SetValue((ModelPanel.Entity:GetAngles().yaw))
                    end  
                    local toggleTurn2 = self:Add("FS_Button")
                    local posx, posy = toggleTurn:GetPos()
                    toggleTurn2:SetPos(posx,(posy+toggleTurn:GetTall())+2)
                    toggleTurn2:SetSize( toggleTurn:GetSize() )
                    toggleTurn2:SetText("Obrót -")
                    function toggleTurn2.DoClick() 
                        ModelPanel.Entity:SetAngles((ModelPanel.Entity:GetAngles()-Angle(0,20,0)))
                        angleSlider:SetValue((ModelPanel.Entity:GetAngles().yaw))
                        
                    end     
                    local applyButton = self:Add("FS_Button")
                    local posx, posy = toggleTurn2:GetPos()
                    applyButton:SetPos(posx,(posy+toggleTurn2:GetTall())+2)
                    applyButton:SetSize( toggleTurn2:GetSize() )
                    applyButton:SetText("Zastosuj")
                    function applyButton.DoClick() 
                        LocalPlayer().bgdelay = LocalPlayer().bgdelay or 0
                        if LocalPlayer().bgdelay>CurTime() then return end
                        LocalPlayer().bgdelay = CurTime()+2                        
                       local t = BodyGroupsConvert(ModelPanel.Entity)
                       if tonumber(t.bodygroups)<=0 then return end
                       net.Start("FS_BodygroupChange")
                       net.WriteTable(t)
                       net.SendToServer()
                    end                
            --Przełączniki obrotu end
