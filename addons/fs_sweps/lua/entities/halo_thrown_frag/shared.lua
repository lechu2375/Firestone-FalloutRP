...
function ENT:Explosion()
	...
	util.BlastDamage(self.Entity, self.Owner, self.Entity:GetPos(), 300, 150)
	...
	for i=1, 30 do 
		...
		if ouchies.Hit then
			...
			if ouchies.Entity == self.Owner then 
				ouchies.Entity:TakeDamage(350 * math.Rand(.85,1.15), self.Owner, self.Entity)
			end
		end
	end
...
function ENT:PhysicsCollide(data,phys)
	...
	local impulse = -data.Speed * data.HitNormal * .1 + (data.OurOldVelocity * -.6)
