...
function ENT:SpawnFunction(ply, tr)
	...
	local ent = ents.Create("m9k_ammo_plasmamines")
...
function ENT:Initialize()
	...
	local model = ("models/Halokiller38/fallout/weapons/mines/plasmamine.mdl")
...
function ENT:Use(activator, caller)
	...
	if (activator:IsPlayer()) and not self.Planted then
		if activator:GetWeapon("weapon_mineplasma") == NULL then
			activator:Give("weapon_mineplasma")
			activator:GiveAmmo(1, "PlasmaMine")
		else		
			activator:GiveAmmo(1, "PlasmaMine")
		end
		self.Entity:Remove()
