...
function ENT:SpawnFunction(ply, tr)
	...
	local ent = ents.Create("m9k_ammo_pulsemines")
...
function ENT:Initialize()
	...
	local model = ("models/Halokiller38/fallout/weapons/mines/pulsemine.mdl")
...
function ENT:Use(activator, caller)
	...
	if (activator:IsPlayer()) and not self.Planted then
		if activator:GetWeapon("weapon_minepulse") == NULL then
			activator:Give("weapon_minepulse")
			activator:GiveAmmo(1, "PulseMine")
		else		
			activator:GiveAmmo(1, "PulseMine")
		end
		self.Entity:Remove()
