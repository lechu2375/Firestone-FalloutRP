...
function ENT:Explosion()
	...
	local effectdata = EffectData()
		...
		util.Effect("m9k_plasma_splode", effectdata)
		self.Entity:EmitSound("weapons/plasmagrenade/fx_explosion_grenade_plasma_high_01.wav", 100, math.random(95, 105), 1, CHAN_WEAPON)
		
	local scorchstart = self.Entity:GetPos() + ((Vector(0,0,1)) * 5)
	local scorchend = self.Entity:GetPos() + ((Vector(0,0,-1)) * 5)
	
	util.BlastDamage(self.Entity, self.Owner, self.Entity:GetPos(), 200, 500)
...
--[[function ENT:OtherExplosion()
	...
	local fire = EffectData()
		...
		fire:SetScale(1)
	util.Effect("m9k_plasma_splode", fire)
	...
	for i=1, 30 do 
		...
		if ouchies.Hit then
			...
			if ouchies.Entity == self.Owner then 
				ouchies.Entity:TakeDamage(500 * math.Rand(1,1.30), self.Owner, self.Entity)
			end
		end
	end
