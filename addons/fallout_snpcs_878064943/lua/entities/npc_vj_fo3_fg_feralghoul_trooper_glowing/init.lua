...
-----------------------------------------------*/
ENT.Model = {"models/fallout/ghoulferal_trooper.mdl"} -- The game will pick a random model from the table when the SNPC is spawned | Add as many as you want
...
local EnemyDistance = self:VJ_GetNearestPointToEntityDistance(self:GetEnemy(),self:GetPos():Distance(self:GetEnemy():GetPos()))
	...
	local randattack_close = math.random(1,2)
		...
		elseif randattack_close == 2 then
			self.AnimTbl_MeleeAttack = {ACT_RANGE_ATTACK1}
			self.MeleeAttackAngleRadius = 70 -- What is the attack angle radius? | 100 = In front of the SNPC | 180 = All around the SNPC
			self.MeleeAttackDamageAngleRadius = 180 -- What is the damage angle radius? | 100 = In front of the SNPC | 180 = All around the SNPC
			self.MeleeAttackDamageDistance = 230
			self.NextAnyAttackTime_Melee = 4 -- How much time until it can use any attack again? | Counted in Seconds
			self.TimeUntilMeleeAttackDamage = false // 1.7 -- This counted in seconds | This calculates the time until it hits something
...
function ENT:CustomOnAcceptInput(key,activator,caller,data)
	...
	elseif key == "event_glowstart" then
		self:EmitSound("npc/ghoulferal/feralghoul_radiate.mp3", 80, 100, 1)
	...
	elseif key == "event_rattack radiation" then
		self:MeleeAttackCode()
		ParticleEffect("radiation_shockwave", self:LocalToWorld(Vector(0,0,40)), self:GetAngles(), self)
		util.ScreenShake(self:GetPos(),12,100,1.5,700)
	
	elseif key == "event_play AttackRadiation" then
		self.MeleeAttackDamageType = DMG_POISON
	
	elseif key == "event_play Attack" then
